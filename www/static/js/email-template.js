...
function DRAGON_DROP(o) {
    ...
    DD.bindDraggables = function() {
        DD.$draggables = $(DD.$draggables); // reselecting
        DD.$noDrags = $(DD.$noDrags);
        ...
        DD.$draggables.off('dragstart').on('dragstart', function(event) {
            ...
            if ($(e.target).hasClass('f_submission')) {
                content = $(content).html('{SUBMISSION::ID}');
            } else if ($(e.target).hasClass('f_submission_date')) {
                content = $(content).html('{SUBMISSION::DATE}');
            } else if ($(e.target).hasClass('f_submission_subject')) {
                content = $(content).html('{SUBMISSION::SUBJECT}');
            } else {
    ...
    DD.bindDropzones = function() {
        //console.log(DD.$dropzones.get(0).selector);
        DD.$dropzones = $(DD.$dropzones); // reselecting
        ...
        DD.$dropzones.off('drop').on('drop', function(event) {
            var e = event.originalEvent;
            //console.log(DD.dropLoad + "dsdsds");
            ...
            sel.removeAllRanges();
            sel.addRange(range);
            $(sel.anchorNode).closest(DD.$dropzones).focus(); // essential
...
function copyEditorContent() {
    document.getElementById("editorHidden").value = document.getElementById("editor").innerHTML;
    document.getElementById("subjectHidden").value = document.getElementById("subject").innerHTML;
    document.getElementById("ccHidden").value = document.getElementById("cc").innerHTML;
    document.getElementById("bccHidden").value = document.getElementById("bcc").innerHTML;
    document.getElementById("recipientHidden").value = document.getElementById("recipient").innerHTML;
    document.getElementById("emailfromHidden").value = document.getElementById("emailfrom").innerHTML;
    document.getElementById("emailReplyToHidden").value = document.getElementById("emailReplyTo").innerHTML;
...
$(function() {
    ...
    $('select[name=formselect]').on("change", function() {
        window.location.href = '/email/' + $(this).val();
    });
    $("#subject, #recipient, #cc, #bcc").keypress(function(e) {
        return e.which != 13;
    });

    $("#html_editor").on("click", function() {
        $("#html_editor").hide();
        $("#simple_editor").show();
        $("#editor").hide();
        $("#editorHTMLHidden").show();
        $(".btn-toolbar .btn-group").hide();
        $(".btn-toolbar .btn-actions").show();
    });

    $("#simple_editor").on("click", function() {
        $("#html_editor").show();
        $("#simple_editor").hide();
        $("#editor").show();
        $("#editorHTMLHidden").hide();
        $(".btn-toolbar .btn-group").show();
    });

    $("#editorHTMLHidden").on("blur", function() {
        var templateHTML = $(this).val();
        var result = templateHTML.match(/<body>[\s\S]*?<\/body>/g);
        var body = result[0];
...
document.getElementById("editor").addEventListener("input", function() {
    var templateHTML = document.getElementById("editorHTMLHidden").value;
    templateHTML = templateHTML.replace(/<body>[\s\S]*?<\/body>/, '<body>' + document.getElementById("editor").innerHTML + '<\/body>');
    $("#editorHTMLHidden").val(templateHTML);
}, false);